...
RESULT=$($BINARY tx pep submit-encrypted-tx $CIPHER $NEW_TARGET_HEIGHT --from $VALIDATOR_2 --gas-prices 1token --gas 300000 --home $CHAIN_DIR/$CHAINID_2 --chain-id $CHAINID_2 --node $CHAIN2_NODE --broadcast-mode sync --keyring-backend test -o json -y)
check_tx_code $RESULT
RESULT=$(wait_for_tx $RESULT)
...
if [ "$EVENT_TYPE" != "new-encrypted-tx-submitted" ] && [ "$TARGET_HEIGHT" != "$NEW_TARGET_HEIGHT" ]; then
  ...
  echo "ERROR: Pep module submit encrypted tx error. Expected tx to submitted without error with target height '$NEW_TARGET_HEIGHT', got '$TARGET_HEIGHT' and '$EVENT_TYPE' | '$CURRENT_BLOCK'"
  echo "ERROR MESSAGE: $(echo "$RESULT" | jq -r '.raw_log')"
